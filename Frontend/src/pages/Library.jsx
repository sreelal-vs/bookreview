...
import { deleteCollectionThunk, getBookCollectionsThunk, getFavouriteBooksThunk, updateFavouriteThunk } from "../Redux/bookCollectionSlice";
...
const Library = () => {
    ...
    const { collections, fav } = useSelector((state) => state.collection);
    ...
    const books = readListBooks.map((item) => {
        const liked = likeMap.get(item.book._id.toString()) || false;
        return {
            ...item,
            likeStatus: liked
        }
                                    ...
                                    <Card className="border-0">
                                        ...
                                        <Card.Body>
                                            <div className="d-flex">
                                                ...
                                                <CollectionDropdown bookId={item.book._id} />

                                                <div className="fav-btn bg-transparent  rounded-2 pt-2 " onClick={() => { handleLike(item.likeStatus, item.book._id) }}>
                                                    <TbHeartFilled className={`mt-1 ${item.likeStatus ? "liked" : "notliked"} ${clickedId == item.book._id ? "animate" : ""}`} size={20} />
                                                </div>
                                            </div>
